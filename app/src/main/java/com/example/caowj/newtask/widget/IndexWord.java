    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN | MotionEvent.ACTION_MOVE:
                ...
                //获取被点击的字幕，然后回调出去
                if (indexPressWord != null && index >= 0 && index < words.length) {
...
//            LogUtil.myD("drawX:" + (everywidth / 2 - width / 2) + "drawY:" + (everyheght * i + (everyheght / 2 + height / 2)));
...
//        LogUtil.myD("everywidth:" + everywidth + "everyheght:" + everyheght + "length:" + length);
    }
